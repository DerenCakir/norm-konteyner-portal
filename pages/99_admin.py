            ...
            try:
                with get_session() as s:
                    ...
                    if existing is not None:
                        ...
                        clear_cached_queries()
                        queue_toast(f"'{new_username}' oluşturuldu.", icon="✅")
    ...
    if create_color_clicked:
        ...
        if not clean_name:
            ...
            try:
                ...
                clear_cached_queries()
                queue_toast(f"'{clean_name}' rengi eklendi.", icon="✅")
                st.rerun()
            except Exception as exc:
                st.error(f"Hata: {exc}")
        ...
        if update_color_clicked:
            ...
            if not clean_name:
                ...
                try:
                    ...
                    clear_cached_queries()
                    queue_toast("Renk güncellendi.", icon="✅")
                    st.rerun()
                except Exception as exc:
                    st.error(f"Hata: {exc}")
        ...
        if selected_color.is_active:
            if st.button("Rengi Sil (Pasifleştir)", key=f"deactivate_color_{selected_color_id}", use_container_width=True):
                try:
                    ...
                    clear_cached_queries()
                    queue_toast("Renk pasifleştirildi.", icon="✅")
                    st.rerun()
                except Exception as exc:
                    st.error(f"Hata: {exc}")
    ...
    if editable_users and selected_user_id != admin_id:
            if st.button(
                ...
                try:
                    ...
                    clear_cached_queries()
                    queue_toast(f"'{selected_user.username}' pasifleştirildi.", icon="✅")
                    st.rerun()
                except Exception as exc:
                    st.error(f"Hata: {exc}")
    ...
    if open_clicked:
        ...
        if closes_at <= current_time:
            ...
            try:
                ...
                clear_cached_queries()
                if late_scope == "Hafta geneli":
                    queue_toast(f"{selected_week} için hafta geneli geç giriş penceresi açıldı.", icon="✅")
                else:
                    queue_toast(f"{selected_week} için kullanıcı özel geç giriş izni açıldı.", icon="✅")
                st.rerun()
            except Exception as exc:
                st.error(f"Hata: {exc}")
        ...
        with st.expander("Seçili Haftanın Tüm Sayımlarını Sil", expanded=False):
            ...
            if bulk_delete_clicked:
                try:
                    ...
                    clear_cached_queries()
                    queue_toast(f"{override_week} haftasındaki {len(submissions)} sayım kaydı silindi.", icon="✅")
                    st.rerun()
                except Exception as exc:
                    st.error(f"Hata: {exc}")
        ...
        if existing_sub is None:
            ...
            if st.button("Bu Sayım Kaydını Sil", key="delete_submission", use_container_width=True):
                try:
                    ...
                    clear_cached_queries()
                    queue_toast("Sayım kaydı silindi.", icon="✅")
                    st.rerun()
                except Exception as exc:
                    st.error(f"Hata: {exc}")
